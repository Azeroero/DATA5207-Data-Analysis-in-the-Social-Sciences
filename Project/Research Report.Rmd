...
```{r data extraction}
# 2016 Census Data
setwd("./Data/NSW_LGA_2016/2016\ Census\ GCP\ Local\ GovernmentAreas\ for\ NSW")



census_data_labels <- read.csv("./Data/NSW_LGA_2016/labels.csv", header=TRUE, sep=',', na.strings="")
census_data <- read.csv("./Data/NSW_LGA_2016/NSW_LGA.csv", header=TRUE, sep=',', na.strings="")
length(census_data_labels$Long)
length(names(census_data))
...
census_data <- merge(dv, census_data_labels, by="region_id")
write.csv(data.merge, 'RData.data.merge.csv')


library(xlsx)
a <- read.xlsx("./Data/BOSCAR_LGA/AlburyLGA.xlsx", sheetName = "Victims")
